...
class LeaveCalendarWindow(QMainWindow):
    ...
    def _build_calendar_side(self) -> QWidget:
        ...
        # Keep the main Fast Encode input unobstructed: it spans the full
        # panel, with year and submit controls directly beneath it.
        fast_layout.addWidget(self.fast_range_edit, 0, 0, 1, 4)
        fast_layout.addWidget(QLabel("YEAR"), 1, 0)
        fast_layout.addWidget(self.fast_year_spin, 1, 1, 1, 2)
        fast_layout.addWidget(self.fast_add_button, 1, 3)
        for index, lock_button in enumerate(self.leave_lock_buttons.values()):
            fast_layout.addWidget(lock_button, 2 + (index // 4), index % 4)
        fast_layout.addWidget(self.mone_list_button, 3, 0, 1, 2)
        fast_layout.addWidget(self.mandatory_leave_button, 3, 2, 1, 2)
        fast_layout.addWidget(self.fast_help, 4, 0, 1, 4)
        self.fast_group.setMaximumHeight(235)
